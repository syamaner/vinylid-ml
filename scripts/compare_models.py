#!/usr/bin/env python3
"""Generate a cross-model comparison report from evaluation results.

Reads ``results/summary.csv`` (written by evaluate.py) and optionally
``results/latency_summary.csv`` (written by run_zero_shot_eval.py).
Produces ``comparison.csv`` and a colour-coded ``comparison.html``.
